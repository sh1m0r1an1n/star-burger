...
**Создание заказа:**
1. Клиент выбирает товары в корзине
2. Заполняет форму с контактными данными и адресом
3. Данные отправляются через API в Django
4. Создается запись в таблице `Order`
5. Для каждого товара создается запись в `OrderItem`

**Обработка заказа менеджером:**
1. Менеджер видит новые заказы в панели управления
2. Система автоматически рассчитывает расстояния до ресторанов
3. Используется кэш геокодера для ускорения расчетов
4. Менеджер выбирает ближайший ресторан
5. Заказ передается в ресторан для приготовления

**Кэширование геокодера:**
- Адреса и координаты сохраняются в таблице `GeocoderCache`
- При повторных запросах координаты берутся из БД
- Это ускоряет загрузку страницы в 100+ раз
- Кэш обновляется автоматически при изменении адресов
